	...
	while (userCommand != "exit") // Keep going indefenitely until stopped...
	{
		...
		else
		{
			...
			else if (command.compare("open") == 0)
			{
				try
				{
					...
					if (initialDeposit >= 0)
					{
						...
						else if (accountType.compare("2") == 0)
						{
						...
						else if (accountType.compare("3") == 0)
						{
							...
							{
								std::cout << "ISA initial balance must be more than 1000 pounds!" << std::endl; // Fulfill the requirement that ISAs start with over £1000
							}
						}
						else
						{
							std::cout << "Invalid account type, try again!" << std::endl;
						}
					}
					else
					{
						std::cout << "Initial deposit cannot be a negative number!" << std::endl;
					}
				}
				catch (...)
				{
			...
			else if (command.compare("withdraw") == 0)
			{
				try
				{
					if (parameters.size() == 2) // Ensure correct parameter count used
					{
						...
						if (accountDatabase.size() > 0) // Prevent withdrawals before any accounts exist
						{
							if (withdrawAmount > 0) // Prevent zero withdrawals, as this makes no sense
							{
								if (selectedAccount->getBalance() - withdrawAmount >= selectedAccount->getMinBalance()) // Ensure overdraft isn't exceeded
								{
						...
						{
							std::cout << "No accounts exist to withdraw from!" << std::endl;
						}
					}
					else
					{
			...
			else if (command.compare("project") == 0) // Project future balance of a savings account (based on its class-specific interest)
			{
				...
				{
					if (accountDatabase.size() > 0) // Prevent projecting before any accounts exist
					{
						if (selectedAccount->getType() != "Current account") // Ensure the account is not a current account, as that doesn't provide Interest Earning in this implementation
						{
							try // To catch invalid input of years
							{
								...
								if (yearsDbl >= 0) // Prevent negative years - it does work with negative time, but there's no reason you'd want to do this (unless it's a mistake, so it's right to prevent this entry)
								{
									...
									std::cout << "\x9C" << CurrencyDblToStr(projectedBalance) << std::endl; // Output with an automatic precision provided by CurrencyDblToStr
								}
								else
								{
									std::cout << "Cannot project negative years!" << std::endl;
								}
							}
							catch (...)
							{
					...
					{
						std::cout << "No accounts exist to project!" << std::endl;
					}
				}
				else
				{
			...
			else if (command.compare("search") == 0) // Stretch task
			{
				if (accountDatabase.size() > 0)
				{
					if (parameters.size() == 2) // Only two parameters can be used here; command and search value
					{
						try // To catch invalid search value
						{
							...
							for (Transaction& selectedTransaction : selectedAccount->searchTransaction(searchValue)) // Go through each match found using the lambda function inside each account
							{
				...
				{
					std::cout << "No accounts exist! Unable to search for transactions!" << std::endl;
				}
			}
			else if (command.compare("exit") != 0) // Finally, ensure this error doesn't show for exit, but after that, it's an unknown command
			{
